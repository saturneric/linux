...
#define AT_CTRL_TUNE_CLK_STOP_EN \
	BIT(16) /* clocks stopped during phase code change */
...
#define PHY_PAD_TXSLEW_CTRL_N_SG2042 \
	0x2 /* Slew control for N-Type pad TX for SG2042 */
...
#define DWCMSHC_SDHCI_CQE_TRNS_MODE \
	(SDHCI_TRNS_MULTI | SDHCI_TRNS_BLK_CNT_EN | SDHCI_TRNS_DMA)
...
struct dwcmshc_pltfm_data {
	...
	int (*init)(struct device *dev, struct sdhci_host *host,
		    struct dwcmshc_priv *dwc_priv);
	void (*postinit)(struct sdhci_host *host,
			 struct dwcmshc_priv *dwc_priv);
};
...
static void dwcmshc_phy_init(struct sdhci_host *host)
{
	...
	if (rxsel == PHY_PAD_RXSEL_1V8) {
		u8 sel = FIELD_PREP(PHY_DLLDL_CNFG_SLV_INPSEL_MASK,
...
static void th1520_sdhci_set_phy(struct sdhci_host *host)
{
	...
	if ((host->mmc->caps2 & emmc_caps) == emmc_caps) {
		emmc_ctrl = sdhci_readw(host, priv->vendor_specific_area1 +
				     ...
				     unsigned int clock)
{
	...
	if (clock <= 52000000) {
		 ...
		 */
		extra = DWCMSHC_EMMC_DLL_DLYENA | DLL_STRBIN_DELAY_NUM_SEL |
	...
	extra = 0x5 << DWCMSHC_EMMC_DLL_START_POINT |
		0x2 << DWCMSHC_EMMC_DLL_INC | DWCMSHC_EMMC_DLL_START;
	sdhci_writel(host, extra, DWCMSHC_EMMC_DLL_CTRL);
	err = readl_poll_timeout(host->ioaddr + DWCMSHC_EMMC_DLL_STATUS0, extra,
				 DLL_LOCK_WO_TMOUT(extra), 1,
				 500 * USEC_PER_MSEC);
	if (err) {
	...
	sdhci_writel(host, extra,
		     dwc_priv->vendor_specific_area1 + DWCMSHC_EMMC_ATCTRL);
	...
	if ((priv->devtype == DWCMSHC_RK3588) &&
		...
		extra = DLL_CMDOUT_SRC_CLK_NEG | DLL_CMDOUT_EN_SRC_CLK_NEG |
			DWCMSHC_EMMC_DLL_DLYENA | DLL_CMDOUT_TAPNUM_90_DEGREES |
			DLL_CMDOUT_TAPNUM_FROM_SW;
		sdhci_writel(host, extra, DECMSHC_EMMC_DLL_CMDOUT);
	}

	extra = DWCMSHC_EMMC_DLL_DLYENA | DLL_TXCLK_TAPNUM_FROM_SW |
		DLL_RXCLK_NO_INVERTER << DWCMSHC_EMMC_DLL_RXCLK_SRCSEL |
		txclk_tapnum;
	sdhci_writel(host, extra, DWCMSHC_EMMC_DLL_TXCLK);

	extra = DWCMSHC_EMMC_DLL_DLYENA | DLL_STRBIN_TAPNUM_DEFAULT |
			       ...
			       struct dwcmshc_priv *dwc_priv)
{
	...
	priv->reset = devm_reset_control_array_get_optional_exclusive(
		mmc_dev(host->mmc));
	...
	if (of_property_read_u8(mmc_dev(host->mmc)->of_node,
				"rockchip,txclk-tapnum", &priv->txclk_tapnum))
...
static void dwcmshc_rk35xx_postinit(struct sdhci_host *host,
				    struct dwcmshc_priv *dwc_priv)
{
	/*
...
static void dwcmshc_rk3576_postinit(struct sdhci_host *host,
				    struct dwcmshc_priv *dwc_priv)
{
...
static int th1520_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	sdhci_writeb(host,
		     ...
		     PHY_ATDL_CNFG_R);
	val = sdhci_readl(host,
			  priv->vendor_specific_area1 + DWCMSHC_EMMC_ATCTRL);
...
static int th1520_init(struct device *dev, struct sdhci_host *host,
		       struct dwcmshc_priv *dwc_priv)
{
...
static void cv18xx_sdhci_reset(struct sdhci_host *host, u8 mask)
{
	...
	val = sdhci_readl(host,
			  priv->vendor_specific_area1 + CV18XX_SDHCI_MSHC_CTRL);
	...
	sdhci_writel(host, val,
		     priv->vendor_specific_area1 + CV18XX_SDHCI_PHY_CONFIG);

	val = (FIELD_PREP(CV18XX_PHY_TX_DLY_MSK, 0) |
	       FIELD_PREP(CV18XX_PHY_TX_SRC_MSK,
			  CV18XX_PHY_TX_SRC_INVERT_CLK_TX) |
	       FIELD_PREP(CV18XX_PHY_RX_DLY_MSK, 0) |
	       FIELD_PREP(CV18XX_PHY_RX_SRC_MSK,
			  CV18XX_PHY_RX_SRC_INVERT_RX_CLK));
	sdhci_writel(host, val,
		     priv->vendor_specific_area1 + CV18XX_SDHCI_PHY_TX_RX_DLY);
}
...
static inline void sg2042_sdhci_phy_init(struct sdhci_host *host)
{
	...
	val = FIELD_PREP(PHY_ATDL_CNFG_INPSEL_MASK,
...
static const struct sdhci_pltfm_data sdhci_dwcmshc_rp1_pdata = {
	...
	.quirks = SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN |
		  SDHCI_QUIRK_BROKEN_CARD_DETECTION,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN |
		   SDHCI_QUIRK2_BROKEN_HS200 | SDHCI_QUIRK2_SPURIOUS_INT_RESP,
};
