...
static void
xe_gt_tlb_invalidation_fence_fini(struct xe_gt_tlb_invalidation_fence *fence)
{
...
static void
__invalidation_fence_signal(struct xe_device *xe,
...
static void
invalidation_fence_signal(struct xe_device *xe,
			  struct xe_gt_tlb_invalidation_fence *fence)
{
	list_del(&fence->link);
...
void xe_gt_tlb_invalidation_fence_signal(
	struct xe_gt_tlb_invalidation_fence *fence)
{
	if (WARN_ON_ONCE(!fence->gt))
		return;
...
static void xe_gt_tlb_fence_timeout(struct work_struct *work)
{
	...
	list_for_each_entry_safe(fence, next,
		...
		s64 since_inval_ms =
			ktime_ms_delta(ktime_get(), fence->invalidation_time);
	...
	if (!list_empty(&gt->tlb_invalidation.pending_fences))
		queue_delayed_work(system_wq, &gt->tlb_invalidation.fence_tdr,
...
#define MAKE_INVAL_OP(type)                                           \
	((type << XE_GUC_TLB_INVAL_TYPE_SHIFT) |                      \
...
int xe_guc_tlb_invalidation_done_handler(struct xe_guc *guc, u32 *msg, u32 len)
{
	...
	if (!list_empty(&gt->tlb_invalidation.pending_fences))
		mod_delayed_work(system_wq, &gt->tlb_invalidation.fence_tdr,
				 tlb_timeout_jiffies(gt));
	else
		cancel_delayed_work(&gt->tlb_invalidation.fence_tdr);
 ...
 * even on error.
 */
void xe_gt_tlb_invalidation_fence_init(
	struct xe_gt *gt, struct xe_gt_tlb_invalidation_fence *fence,
	bool stack)
{
	...
	dma_fence_init(&fence->base, &invalidation_fence_ops,
		       &gt->tlb_invalidation.lock, dma_fence_context_alloc(1),
		       1);
